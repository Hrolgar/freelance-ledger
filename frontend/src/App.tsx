...
function SettingsLink({ mobile = false }: { mobile?: boolean }) {
  return (
    <NavLink
      to="/settings"
      className={({ isActive }) =>
        `flex ${mobile ? 'size-11' : 'size-7'} items-center justify-center rounded-md transition-colors ${
...
function AppShell() {
  ...
  const closeDrawerOnNav = (event: MouseEvent<HTMLElement>) => {
  ...
  return (
    <div className="min-h-screen lg:flex" style={{ background: 'var(--bg-base)', color: 'var(--text-primary)' }}>
      <header
        className="sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-[var(--border-faint)] bg-[var(--bg-base)] px-3 lg:hidden"
      >
        <button
          type="button"
          className="flex size-11 items-center justify-center rounded-md text-[var(--text-primary)] transition-colors hover:bg-[var(--bg-surface)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
      ...
      <div className={`fixed inset-0 z-40 lg:hidden ${drawerOpen ? 'pointer-events-auto' : 'pointer-events-none'}`} aria-hidden={!drawerOpen}>
        <button
          type="button"
        ...
        />
        <div
          className={`relative flex h-full w-[260px] flex-col transition-transform duration-200 ease-out ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        >
          ...
          <nav className="flex-1 px-3 py-5 overflow-y-auto" onClick={closeDrawerOnNav}>
            <NavigationGroups />
          </nav>
        </div>
      </div>
