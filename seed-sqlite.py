...
def insert_project(client, name, platform, currency, fee_pct, status, awarded=None, completed=None, notes=None, client_id=None):
    c.execute("""INSERT INTO Projects (ClientName, ProjectName, Platform, Currency, FeePercentage, Status, DateAwarded, DateCompleted, Notes, ClientId)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
              (client, name, PLATFORM[platform], CURRENCY[currency], str(fee_pct), PROJECT_STATUS[status], awarded, completed, notes, client_id))
    return c.lastrowid
...
def insert_client(name, email=None, country=None, timezone=None, freelancer_id=None, upwork_id=None, aliases=None, notes=None):
    c.execute("""INSERT INTO Clients (Name, Email, Country, Timezone, FreelancerId, UpworkId, Aliases, Notes, Phone)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL)""",
              (name, email, country, timezone, freelancer_id, upwork_id, aliases, notes))
    return c.lastrowid

print("Seeding clients...")
nick_id = insert_client("Nick Owens", email="nickowens111@mail.com", country="UK", freelancer_id="nick111nick111", aliases="nick111nick111", notes="First client. Repeat business. Future work direct (no Freelancer fees).")
sahil_id = insert_client("Sahil", country="Canada", timezone="America/Toronto", freelancer_id="sahil1705", aliases="sahil1705")
maroan_id = insert_client("Maroan", country="Saudi Arabia", freelancer_id="maroan10MB", aliases="maroan10MB", notes="MB Store. Wants TikTok automation.")
maaz_id = insert_client("Maaz", freelancer_id="maazm24", aliases="maazm24", notes="Original Amazon bot quote. Went silent.")
gautham_id = insert_client("Gautham", freelancer_id="gauthamm0", aliases="gauthamm0", notes="Claude Superagent inquiry.")

print("Seeding projects...")

# 1. Nick — Original bot (£180, 2x£90)
pid = insert_project("nick111nick111", "Claude Code Investment Bot", "Freelancer", "GBP", 10, "Paid", "2026-03-31", "2026-03-31", "First freelance delivery. Project ID 40332834.", client_id=nick_id)
...
pid = insert_project("nick111nick111", "TaoBot Upgrade — Fallback RPC", "Freelancer", "GBP", 10, "Paid", "2026-04-03", "2026-04-04", "Project ID 40346281. Bulk query, cycle 200s→7s.", client_id=nick_id)
...
pid = insert_project("nick111nick111", "Surge Alerts + Subnet Ranking", "Freelancer", "GBP", 10, "InProgress", "2026-04-07", None, "£80 milestone pending release after Nick confirms.", client_id=nick_id)
insert_milestone(pid, "Surge alerts + ranking", 80, "GBP", "Funded", 1)

# 4. Sahil — Amazon Shift Bot (45k INR)
pid = insert_project("sahil1705", "Amazon Shift Booking Bot", "Freelancer", "INR", 10, "InProgress", "2026-04-07", None, "Project ID 40350500. Multi-location deployed. Friday 2026-04-11 test day.", client_id=sahil_id)
...
pid = insert_project("maroan10MB", "TikTok Mass Engagement Bot", "Freelancer", "USD", 10, "Awarded", "2026-04-08", None, "Project ID 40354991. Waiting for client infra.", client_id=maroan_id)
...
pid = insert_project("maazm24", "Amazon Shift Bot", "Freelancer", "CAD", 10, "Quoted", None, None, "Original quote. Client went silent.", client_id=maaz_id)
insert_milestone(pid, "Full delivery", 610, "CAD", "Pending", 1)

# 7. Gautham — Claude Superagent (quoted)
pid = insert_project("gauthamm0", "Claude Superagent", "Freelancer", "INR", 10, "Quoted", None, None, "55k INR quote. Client asking questions.", client_id=gautham_id)
